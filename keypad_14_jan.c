...
uint ledBLUEPin = 28; // Pino conectado ao LED Azul
uint ledGREENPin = 27; // Pino conectado ao LED Verde
uint ledREDPin = 26; // Pino conectado ao LED Vermelho
...
void controlOutputs(char key) {
    ...
    if (key == '#') {
        gpio_put(buzzerPin, 1); // Ativa o buzzer
        sleep_ms(250); // Esperar 500 ms
        gpio_put(buzzerPin, 0); //Desativa o buzzer
        sleep_ms(250); // Esperar 500 ms
    }
}
